...
export const ManualEntry = ({ onClose }: ManualEntryProps) => {
  ...
  const handleAddSteps = () => {
    ...
    if (stepCount > 0) {
      motionSensor.addSteps(stepCount);
      storageService.updateTodaysSteps(motionSensor.getSteps());
      toast({
        title: "Steps added",
  ...
  const handleAddActivity = () => {
    ...
    if (activityName && durationNum > 0 && caloriesNum > 0) {
      ...
      toast({
        ...
        description: `Added ${activityName} (${durationNum} min, ${caloriesNum} cal)`,
      });
      onClose();
    }
  };
  ...
  const handleActivityChange = (value: string) => {
    ...
    if (activityOption && duration) {
      setCalories((activityOption.caloriesPerMinute * parseInt(duration)).toString());
    }
  };
  ...
  const handleDurationChange = (value: string) => {
    ...
    if (activityOption && value) {
      setCalories((activityOption.caloriesPerMinute * parseInt(value)).toString());
    }
  };
        ...
        <div className="p-4 space-y-4">
          ...
          {entryType === 'steps' ? (
            <div className="space-y-4">
              <div>
                <Label htmlFor="steps" className="text-foreground">Number of Steps</Label>
                <Input
                  id="steps"
                  type="number"
                  value={steps}
            ...
            <div className="space-y-4">
              <div>
                <Label htmlFor="activity-select" className="text-foreground">Activity Type</Label>
              ...
              {activity === 'Custom' && (
                <div>
                  <Label htmlFor="custom-activity" className="text-foreground">Custom Activity</Label>
                  <Input
                    id="custom-activity"
              ...
              <div className="flex gap-2">
                ...
                <Button 
                  ...
                  className="flex-1"
                  disabled={!activity || !duration || !calories || (activity === 'Custom' && !customActivity)}
                >
                  <Plus className="w-4 h-4 mr-2" />
                  Add Activity
                </Button>
